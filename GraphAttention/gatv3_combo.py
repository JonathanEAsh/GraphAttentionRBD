...
l = []
for files in os.listdir('binary/saved_models_combo/'):
...
if len(l) > 0:
    ...
    recent_model = l[0][0]
    model = torch.load('binary/saved_models_combo/'+recent_model)
    ...
    for epoch in range(l[0][1], num_epochs):
        ...
        if (epoch+1) % 25 == 0:
            ...
            with open('binary/model_out_combo/'+model_name,'w') as f:
            ...
            torch.save(model, 'binary/saved_models_combo/'+model_name+'.pth')
            model.train()
else:
...
plt.savefig('binary/loss_plots_combo/'+model_name+'.png')