...
import numpy as np
model_name = 'model_0.0_layers_3_epochs_200_lr_0.001_bs_100_acc_0.927_train_0.971'
drop = float(model_name.split('_')[1])
#chn=int(args.chn)
act1=LeakyReLU(0.1)
act2=Sigmoid()
...
def test(loader):
    ...
    for data in loader:  # Iterate in batches over the training/test dataset.
        ...
        # print(out)
        # print(out[:,0])
        # bind_probs = bind_probs + list(out[:,0].cpu())
        # preds = preds + list(pred.cpu())
        names = names + list(name)
        #print(list(pred))
        # true = true + list(g.y.cpu())
        bind_probs = bind_probs + [tensor.item() for tensor in out[:,1]]
        pred_list = pred_list + [tensor.item() for tensor in pred]
        true = true + [tensor.item() for tensor in g.y]
        print(len(true))
    # true_cpu=[]
    # preds_cpu=[]
    ...
    return names, true, pred_list, bind_probs
...
print(device)
# tr_r1_te_r2
# model_name = 'model_0.0_layers_3_epochs_150_lr_0.01_bs_200_acc_0.767_train_0.804.pth'
#combo
#model_name = 'model_0.0_layers_3_epochs_75_lr_0.001_bs_200_acc_0.826_train_0.859.pth'

for files in os.listdir('best_split_model_nooh/'):
    model_name = files
    bs=int(model_name.split('_')[9])
    print(model_name)
    print(bs)
    model = torch.load('best_split_model_nooh/'+model_name)
    model.eval()

    train_loader=DataLoader(set_train, batch_size=bs, shuffle=False)
    val_loader=DataLoader(set_val, batch_size=bs, shuffle=False)
    test_loader=DataLoader(set_test, batch_size=bs, shuffle=False)

    print('=== TRAIN ===')
    train_names, train_true, train_pred, train_prob = test(train_loader)
    df1 = pd.DataFrame(zip(train_names, train_true, train_pred, train_prob), columns=['name','true','pred','prob'])
    df1 = df1.sort_values(by=['prob'], ascending=False)
    df1.to_csv('split_neighbor_nooh_train_probs.csv',index=False)

    print('=== VAL ===')
    val_names, val_true, val_pred, val_prob = test(val_loader)
    df2 = pd.DataFrame(zip(val_names, val_true, val_pred, val_prob), columns=['name','true','pred','prob'])
    df2 = df2.sort_values(by=['prob'], ascending=False)
    df2.to_csv('split_neighbor_nooh_val_probs.csv',index=False)
    
    print('=== TEST ===')
    test_names, test_true, test_pred, test_prob = test(test_loader)
    df3 = pd.DataFrame(zip(test_names, test_true, test_pred, test_prob), columns=['name','true','pred','prob'])
    df3 = df3.sort_values(by=['prob'], ascending=False)
    df3.to_csv('split_neighbor_nooh_test_probs.csv',index=False)
