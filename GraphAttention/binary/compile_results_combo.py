...
best_test=[]
best_train=[]
for files in os.listdir('model_out_combo/'):
	#if 'knn' in files and '4mer' in files:
	print(files)
	with open('model_out_combo/'+files) as f:
		lines=f.readlines()
		x = lines[0]
...
df=pd.DataFrame(zip(names, drop,layers,epochs,lr,bs,train,val),columns=['names','drop','layers','epochs','lr','bs','train','val'])
#df=pd.concat([old_df, df])
df=df.sort_values(by='val',axis=0,ascending=False)
df.to_csv('neighbor_combo_binary_noie.csv',index=False)
