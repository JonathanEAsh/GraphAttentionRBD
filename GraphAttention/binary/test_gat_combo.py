...
import numpy as np
model_name = 'model_0.2_layers_3_epochs_25_lr_0.001_bs_200_acc_0.949_train_0.951'
...
class GCN(torch.nn.Module):
    def __init__(self, hidden_channels):
        ...
        self.conv1 = GATv2Conv(22, 20, edge_dim=2, heads=2)
...
set_train=RBDDataset_train(root='../train_r1_r2/train/')
#set_train.process()
set_val=RBDDataset_val(root='../train_r1_r2/val/')
#set_val.process()
...
for files in os.listdir('best_combo_model_noie/'):
    model_name = files
    bs=int(model_name.split('_')[9])
    print(model_name)
    print(bs)
    model = torch.load('best_combo_model_noie/'+model_name)
    model.eval()
    ...
    df1.to_csv('combo_neighbor_noie_train_probs.csv',index=False)
    ...
    df2.to_csv('combo_neighbor_noie_val_probs.csv',index=False)
