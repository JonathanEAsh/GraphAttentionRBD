...
for files in os.listdir('best_model_tr_r1_te_r2/'):
	model_name = files
...
def main():
	...
	for files in os.listdir('best_model_tr_r1_te_r2/'):
		...
		model.load_state_dict(torch.load('best_model_tr_r1_te_r2/'+model_name))
		model.eval()
		model.to(device=device)

		print(' === Train === ')
		df_train = pd.read_csv('round_1_train_binary.csv')
		...
		df_train.to_csv('rbd_attention_split_train_probs.csv',index=False)
		...
		df_val.to_csv('rbd_attention_split_val_probs.csv',index=False)
