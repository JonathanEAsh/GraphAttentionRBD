...
def main():
	...
	# TRAIN
	df_train = pd.read_csv('round_1_train_resample.csv')
	df_val = pd.read_csv('round_1_val.csv')
	...
	for i in range(num_epochs):
		...
		if (i + 1) % 25 == 0:
			...
			with open('quick_model_out/'+model_name+'.txt','w') as f:
				f.write(str(train_acc_bal)+','+str(val_acc_bal)+','+str(test_acc_bal))
				#f.write(str(train_acc_bal)+','+str(val_acc_bal))
			torch.save(model.state_dict(),'quick_save/'+model_name+'.pt')
			model.train()
